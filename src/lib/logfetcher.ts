...
const logfetcher = {
	...
	fetchLogList: async function (profile: DwJson, debug?: boolean, logpath = ''): Promise<string> {
		try {
			...
			if (profile.log_security && profile.log_security === "all") {
				...
				let resSecurity;
				let resData;
				resSecurity = await this.makeRequest(profile, 'GET', logpath, headers, debug, securityLog);
				resData = resSecurity.data.concat(resSite.data);
			...
			logger.log(logger.error, 'Fetching log list failed with error: ' + err.message + logpath);
			switch (err.status) {
				case 401:
	...
	fetchFileSize: async function (profile: DwJson, logobj: LogFile): Promise<number> {
		...
		try {
			...
			let res = await this.makeRequest(profile, 'HEAD', logobj.log, null, logobj.debug, logobj.logSource === 'Securitylogs' ? true : false);
	...
	fetchLogContent: async function (profile: DwJson, logobj: LogFile): Promise<[LogFile, string]> {
		try {
			...
			let res = await this.makeRequest(profile, 'GET', logobj.log, headers, logobj.debug, logobj.logSource === 'Securitylogs' ? true : false);
			logger.log(logger.debug, `Fetching contents from ${logobj.log} source: ${logobj.logSource} retured status code ${res.status}`, logobj.debug);
			if (res.status === 206) {
				if (logobj.size < 0) {
			...
			if (err.response) {
				logger.log(logger.debug, `Fetching contents from ${logobj.log} source: ${logobj.logSource} returned status code ${err.response.status}`, logobj.debug);
			}
			if (!err.response || err.response.status !== 416) {
				this.errorcount = this.errorcount + 1;
				if (this.errorcount > 1) {
					logger.log(logger.error, `Error fetching contents from ${logobj.log} source(${logobj.logSource}): ${err.message} (error count ${this.errorcount})`);
				} else {
					// don't be too verbose, just retry if this was the first error
					logger.log(logger.debug, `Error fetching contents from ${logobj.log} source(${logobj.logSource}): ${err.message} (error count ${this.errorcount})`);
				}
			}
		}
		return [logobj, ''];
