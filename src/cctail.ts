...
let getThatLogList = async function (profile: DwJson, filesuffix = ".log"): Promise<LogFile[]> {
	...
	let regexp = new RegExp(`<a href="/on/demandware.servlet/webdav/Sites/(.*?)/(.*?)">[\\s\\S\\&\\?]*?<td align="right">(?:<tt>)?(.*?)(?:<\\/tt>)?</td>[\\s\\S\\&\\?]*?<td align="right"><tt>(.*?)</tt></td>`, 'gim');
	let match = regexp.exec(data);
	while (match != null) {
		let filedate = moment.utc(match[4]);
		if (match[2].substr(-4) === filesuffix && filedate.isSame(moment.utc(), 'day')) {
			fileobjs.push({
				log: match[2],
				size_string: match[3],
				date: moment.utc(match[4]),
				debug: debug,
				logSource: match[1]
			});
			logger.log(logger.debug, `Available Log: ${match[2]},log size: ${match[3]}, log source: ${match[1]}`, debug);
		}
